...
Imports [OpenStreetMap](https://www.openstreetmap.org) data (roads, rivers, lakes, forests, fields/plantations, trees, buildings) into a map file for the [Canvas of Kings](https://store.steampowered.com/) editor (`.mommap`). Built to turn a real-world place into the basis for a tabletop RPG map, **not for accurate cartography**.
